...
tile_init(struct map *m) {
	m->tiles = (struct tile *)malloc(m->tile_sz * sizeof(struct tile));
	int x, y;
	for (x = 0; x <= m->max_x_index; x++) {
		for (y = 0; y <= m->max_y_index; y++) {
			...
			tl->headers = (struct dlink **)malloc(tl->size * sizeof(struct dlink *));
...
tile_level(struct tile* tl,int level) {
	if (level >= tl->size) {
		...
		int osize = tl->size;		
		tl->headers = (struct dlink **)malloc(nsize * sizeof(struct dlink*));
	...
	if (tl->headers[level] == NULL) {
		tl->headers[level] = (struct dlink *)malloc(sizeof(struct dlink));
...
_aoi_new(lua_State *L) {
	...
	struct aoi_context *aoictx = (struct aoi_context*)malloc(sizeof(*aoictx));
	memset(aoictx,0,sizeof(*aoictx));

	aoictx->pool = (struct objectpool*)malloc(sizeof(*aoictx->pool));
...
_aoi_delete(lua_State *L) {
	struct aoi_context *aoi = (struct aoi_context*)lua_touserdata(L, 1);
...
_aoi_enter(lua_State *L) {
	...
	if (aoi->pool->freelist) {
		...
		struct objectpool_list *opl = (struct objectpool_list*)malloc(sizeof(*opl));
...
_aoi_leave(lua_State *L) {
	struct aoi_context *aoi = (struct aoi_context*)lua_touserdata(L, 1);
	struct object *obj = (struct object*)lua_touserdata(L, 2);
